...
const courtPlus = document.getElementById("courtPlus");
const courtMinus = document.getElementById("courtMinus");
const courtSummary = document.getElementById("courtSummary");

/* COURT STATE */
const courtPreferences = { 1: "OPEN" };
const COURT_TYPES = ["MD", "LD", "XD", "OPEN"];
...
function renderCourtSummary() {
  ...
  for (let i = 1; i <= numCourts; i++) {
    const type = courtPreferences[i] || "OPEN";
    const img = document.createElement("img");
    img.src = `${type}.png`;
    img.title = `Court ${i}: ${type}`;
    courtSummary.appendChild(img);
  }
}
...
function renderCourtConfig() {
  ...
  for (let i = 1; i <= numCourts; i++) {
    ...
    COURT_TYPES.forEach(type => {
      const img = document.createElement("img");
      img.src = `${type}.png`;
      img.className = "court-icon";
...
courtPlus.onclick = () => {
  numCourts++;
  courtPreferences[numCourts] = "OPEN";
