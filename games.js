...
function overlapsTooMuch(pair1, pair2, lastGame) {
  if (!lastGame) return false;

  const lastSet = new Set(lastGame);
  let overlap = 0;

  for (const p of pair1) if (lastSet.has(p)) overlap++;
  for (const p of pair2) if (lastSet.has(p)) overlap++;

  // same 4 players or 3/4 overlap → bad
  return overlap >= 3;
}
...
function AischedulerNextRound(schedulerState) {
  ...
  // 🔥 THIS LOOP IS THE IMPORTANT FIX
  for (const match of matchupScores) {
    ...
    const lastGame = lastRound?.[gameIndex];

    // 🔥 NEW CONDITION
    if (
      overlapsTooMuch(pair1, pair2, lastGame)
    ) {
      continue; // skip if alternatives exist
    }
...
function bkAischedulerNextRound(schedulerState) {
  const {
    activeplayers,
    numCourts,
    fixedPairs,
